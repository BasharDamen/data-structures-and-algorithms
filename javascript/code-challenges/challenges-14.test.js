...
const screenForNames = (arr) => {
  // Solution code here...
  let reg = /^((Mr||Ms||Mrs||Dr).\s)\w+/g;
  return arr.filter((elem)=>{

    return reg.test(elem) ? elem : false;
  })
}
...
const toTitleCase = (arr) => {
  ...
  for (var i = 0; i < arr.length; i++) {
    let newWord = arr[i].charAt(0).toUpperCase() + arr[i].slice(1);
...
let biggerThanLuke = (arr) => {
  ...
  let newArr = [];
  arr.map((item) => {
    item.mass > lukaMass ? newArr.push(item.name) : false;
  });
